...
SkillBridge Runtime is a universal skill runtime for existing AI agents. It lets any agent use standard `SKILL.md` skill packages without re-implementing skill parsing, routing, progressive loading, resource reading, script execution, or runtime tracing.
...
SkillBridge Runtime 是一个面向已有 Agent 的通用 Skill 运行时。它让任何 Agent 无需重新实现 Skill 解析、路由、渐进加载、资源读取、脚本执行和审计逻辑，也能直接使用标准 `SKILL.md` 技能包。
